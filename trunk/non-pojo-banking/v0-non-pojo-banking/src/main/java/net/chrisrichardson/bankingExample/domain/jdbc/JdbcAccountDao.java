...
public class JdbcAccountDao implements AccountDao {
  ...
  public void addAccount(Account account) {
    ...
    try {
      ...
      ps
          .setTimestamp(index++, new Timestamp(account.getDateOpened()
              .getTime()));
      ...
      JdbcConnectionManager.getInstance().cleanUp(con, ps);
    }
  }
  ...
  public Account findAccount(String accountid) {
    ...
    try {
      ...
      Account account = new Account(rs.getInt("account_id"), rs
          ...
          .getInt("overdraftpolicy"), new Date(rs.getTimestamp("dateopened")
          .getTime()), rs.getDouble("requiredyearsopen"), rs.getDouble("limit"));
      Assert.isTrue(!rs.next());
      return account;
    } catch (SQLException e) {
      throw new RuntimeException(e);
    } finally {
      JdbcConnectionManager.getInstance().cleanUp(con, ps, rs);
    }
